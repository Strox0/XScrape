...
public:
	...
	Scraper(_Data& data);
	Scraper(_Data& data, const std::string& out_folder_name);
	~Scraper();
...
private:
	...
	void SaveImage(void* data, size_t data_size, const std::string& extension);
	...
	bool CreateRequest(boost::asio::ssl::stream<tcp::socket>& stream, boost::beast::http::response_parser<BodyType>& response, boost::beast::flat_buffer& buffer, bool show_progress = false, int max_attempts = 10, bool only_header = false);
...
private:
	...
	std::unique_ptr<boost::asio::ssl::stream<tcp::socket>> m_stream;
